...
class SimpleChangeWatcherSample {
  ...
  static SpannerDatabaseChangeWatcher createWatcher(Spanner spanner, DatabaseId databaseId) {
    ...
    SpannerDatabaseChangeWatcher watcher2 =
    ...
    SpannerDatabaseChangeWatcher watcher =
            SpannerDatabaseTailer.newBuilder(spanner, databaseId).includeTables("Singers", "Albums").build();
    ...
    watcher.addCallback(
        new RowChangeCallback() {
          ...
          public void rowChange(TableId table, Row row, Timestamp commitTimestamp) {
            ...
            {
              System.out.printf("in the looppppp");
              SampleData.Singer singertest = new SampleData.Singer(row.getLong(0), row.getString(1), row.getString(2));
              SampleData.writeExampleData2(spanner.getDatabaseClient(databaseId), singertest);
            }
          }
        });
